...
👍建议英语好的朋友直接阅读[Dave大神的原文](https://dave.cheney.net/practical-go/presentations/qcon-china.html)，原滋原味，容易理解。

英语不是那么自信的朋友，读翻译版本可以快速理解，不理解的地方可以翻到英文地方去看看，如果有问题随意`Issue`，欢迎PR。

最方便阅读的方式是`fork`到自己的仓库，同时顺便修改，然后PR。

本仓库`fork`自[llitfkitfk/go-best-practice](https://github.com/llitfkitfk/go-best-practice)，并在此基础上进行了小量修改和增加了注释，注释为括号内*斜体部分*。
...
* **长变量名称需要证明自己的合理性; 名称越长，需要提供的价值越高。冗长的名称与页面上的重量相比，信息量较小**。
* 请勿在变量名称中包含类型名称。
* 常量应该描述它们持有的值，而不是该如何使用。
* 对于循环和分支使用单字母变量，参数和返回值使用1个单词，函数和包级别声明使用多个单词。
* 方法、接口和包使用1个单词。(*比如，Reader, Writer, fmt.Println*)
...
> 贴士: 
像对文档分段一样，使用用空行对函数分段。 在`AverageAge`中，按顺序共有三个操作。 第一个是前提条件，检查`people`是否为空，第二个是`sum`和`count`的累积，最后是平均值的计算。
...
```
我认为它不是，因为就此事而论, `i`和`index`的范围很大可能上仅限于for循环的主体，前者的额外冗长性(指`index`)几乎没有增加对于程序的理解。
...
在此示例中，`oid`是`SNMP Object ID`的缩写，如果将其缩短为`o`，意味着程序员必须要阅读文档才能理解这个符号的含义。 类似地将`index`替换成`i`, 模糊了`i`所代表的含义：`SNMP`消息中，`index`指每个`OID`的子值。


> 贴士: 不要在同一声明中，同时使用长和短形式的参数名称。
...
类似地，对于方法接收器: 在该类型的每个方法上使用相同的接收者名称。 在这种类型的方法内部可以使读者更容易使用。（*注：`func (p *Person)Name()string{}`其中的`p`为方法接收器名称*）
...
thing := &Thing{}
```
与`new(Thing)`相同，这就是为什么一些Go程序员对重复感到不满。 然而，这意味着我们使用指向`Thing{}`的指针初始化了`thing`，也就是`Thing`的零值。（*注意：`Thing`的零值并不是`nil`，而是其中字段的值为各类型的零值。当我创建一个对象，并得到该对象的地址时，我最喜欢这种做法。*）
...
**除了更容易理解之外，较小的函数更易于隔离测试，将代码隔离到函数中，其名称可能是所需的所有文档**。(*注：即函数名就表明了该函数的作用*)
...
一个好的Go语言包应该具有低程度的源码级耦合，这样随着项目的增长，对一个包的更改不会造成跨代码库级联（*有没有深痛的领悟*）。 这种重构严格限制了代码库的变化率以及在该代码库中工作的成员的生产率，如同世界末日。

在本节中，我们将讨论如何设计包，包括包的名称，命名类型以及编写方法和函数的技巧。

#### 4.1. 一个好的包从它的名字开始

编写一个好的Go语言包从包的名称开始。将你的包名用**一个词**来描述它。

正如我在上一节中谈到变量的名称一样，包的名称也非常重要。我遵循的经验法则不是“我应该在这个包中放入什么类型的？”。相反，我要问是“该包提供的服务是什么？”通常这个问题的答案不是“这个包提供`X`类型”，而是“这个包提供`HTTP`”。

> 贴士:
以包所提供的内容来命名，而不是它包含的内容。(*注：使用包的功能命名*)

#### 4.1.1. 好的包名应该是唯一的。

在项目中，每个包名称应该是唯一的。使用包的用途命名的建议很容易理解，如果你发现有两个包需要用相同名称，它可能是:
1. 包的名称太通用了。
